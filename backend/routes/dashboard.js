...
/**
 * @swagger
 * /saveDashboardLayout/{dashboardId}:
 *   post:
 ...
 */
router.post('/saveDashboardLayout/:dashboardId', async (req, res) => {
  try {
    ...
    if (existingDashboard) {
      ...
      res
        .status(200)
        .json({ message: 'Dashboard layouts updated successfully' });
    } else {
      ...
      res
        .status(201)
        .json({ message: 'New dashboard and layouts created successfully' });
    }
  } catch (error) {
    console.error(error);
 ...
 *                   example: Detailed error message
 */
router.get('/getDashboardLayout/:dashboardId', async (req, res) => {
  try {
    const { dashboardId } = req.params;
