...
export const getDevicePublishTelemetryCommands = async (deviceId: string) => {
...
export const saveDevice = async (device: Device, accessToken?: string) => {
...
export const assignDeviceToEdge = async (edgeId: string, deviceId: string) => {
...
export const getFilteredDevices = async (id: string) => {
    ...
    const filteredDevices = response.data.data.filter(
        (device: Device) => device.label == id
    );
    return filteredDevices;
};

export const updateDeviceLabels = async (body: {
    id: string;
    devices: string[];
}) => {
    const response = await mongoAPI.post(`/postgres/updatedevices`, body);
    return response;
};
